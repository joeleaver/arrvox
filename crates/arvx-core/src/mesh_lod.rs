...
/// `ARVX_MESH_LOD_LEVELS=1..=LOD_LEVELS_MAX` at bake time to
...
/// `ARVX_MESH_LOD_LEVELS` override. The DAG converges naturally
...
/// `ARVX_MESH_LOD_LEVELS` once and clamps to `1..=LOD_LEVELS_MAX`;
/// falls back to the [`LOD_LEVELS`] compile-time default.
fn lod_levels_runtime() -> usize {
    std::env::var("ARVX_MESH_LOD_LEVELS")
        .ok()
...
/// count. Reads `ARVX_MESH_LOD_LEVELS` (clamped 1..=LOD_LEVELS_MAX)
