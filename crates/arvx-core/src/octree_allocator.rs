...
/// `ArvxScene::upload_geometry`: `(node, prefilter_id, 0, 0)` × u32 =
...
/// `ArvxScene::upload_geometry`. Keeping them as two separate Vecs on the
/// CPU side lets the rest of arvx-core treat them as independent buffers;
...
impl OctreeAllocator {
    ...
    /// `ArvxSceneManager::walk_snapshot` to hand octree data to the
...
mod tests {
    ...
    fn gpu_lookup_on_packed_rkp() {
        // Load bunny .arvx, allocate into packed buffer (with a dummy before it to
        // force non-zero offset), verify GPU-style position lookups match.
        let path = "/home/joe/dev/rkifield_game/splat5/assets/models/bunny_pbr/scene.arvx";
        if !std::path::Path::new(path).exists() {
            eprintln!("Skipping packed .arvx test — file not found");
            return;
        }
        ...
        let header = match crate::asset_file::read_rkp_header(&mut reader) {
            Ok(h) => h,
            Err(e) => { eprintln!("Skipping packed .arvx test — header error: {e}"); return; }
        };
