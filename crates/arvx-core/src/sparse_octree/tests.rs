...
fn gpu_lookup_matches_rkp_file() {
    // Test with an actual .arvx file if available.
    let path = "/home/joe/dev/rkifield_game/splat5/assets/models/bunny_pbr/scene.arvx";
    if !std::path::Path::new(path).exists() {
        eprintln!("Skipping .arvx test — file not found: {path}");
        return;
    }
    ...
    let header = match crate::asset_file::read_rkp_header(&mut reader) {
        Ok(h) => h,
        Err(e) => { eprintln!("Skipping .arvx test — header error: {e}"); return; }
    };
    ...
    eprintln!("GPU lookup .arvx test: {total} leaves, {mismatches} mismatches (extent={extent}, depth={depth}, vs={vs})");
