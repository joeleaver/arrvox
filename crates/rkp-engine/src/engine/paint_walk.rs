...
use glam::Vec3;
use rkp_core::brick_pool::{BRICK_CELLS, BRICK_DIM, BRICK_INTERIOR};
use rkp_core::sparse_octree::{brick_id, is_brick, is_leaf, leaf_slot, EMPTY_NODE, INTERIOR_NODE};
use rkp_core::{Aabb, LeafAttr, LeafAttrOverlay};
use rkp_render::rkp_scene_manager::WalkSnapshot;
use rkp_render::shader_composer::UserShaderInfo;
...
impl PaintWalkWorker {
    pub fn spawn() -> Self {
        ...
        let handle = std::thread::Builder::new()
            .name("rkp-paint-walk".to_string())
            .spawn(move || worker_loop(inbox_rx, outbox_tx))
            .expect("spawn rkp-paint-walk thread");
        Self {
