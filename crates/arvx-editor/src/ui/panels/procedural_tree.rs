...
use arvx_engine::procedural_snapshot::{ProceduralNodeKind, ProceduralSnapshot};
            ...
            ContextMenu {
                    ...
                    div {
                        ...
                        ondrop: move || {
                            ...
                            if let Some(src) = drag.take() {
                                ...
                                let _ = cmd_tx.get().send(arvx_engine::EngineCommand::MoveProceduralNode {
                                    node_id: src,
                        ...
                        onclick: move || {
                            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SelectProceduralNode {
                ...
                ContextMenuDropdown {
                    {context_menu_item(__scope, "Duplicate", TablerIcon::Copy, {
                        let cmd_tx = cmd_tx;
                        move || {
                            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::DuplicateProceduralNode {
                                node_id,
                            });
                        }
                    })}
                            ...
                            {context_menu_item(__scope, "Change to Union", TablerIcon::CirclePlus, {
                                ...
                                move || {
                                    let _ = cmd_tx.get().send(
                                        arvx_engine::EngineCommand::SetProceduralNodeCombinator {
                                            node_id,
...
fn add_menu_item(
    ...
    rsx! {
        DropdownMenuItem {
            left_section: icon,
            onclick: move || {
                let _ = cmd_tx.get().send(arvx_engine::EngineCommand::AddProceduralNode {
