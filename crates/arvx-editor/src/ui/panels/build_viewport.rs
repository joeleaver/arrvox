...
pub fn BuildViewport() -> NodeHandle {
    ...
    let tree_selected_node = Memo::new(move || tree_snapshot.get().selected_node);
    {
        ...
        __scope.create_effect(move || {
            ...
            let _ = cmd_tx.send(arvx_engine::EngineCommand::SetViewportVisible {
                id: PANEL_VIEWPORT,
                visible,
            });
        });
    ...
    // selected procedural through regardless of its layer bit.
    {
        ...
        __scope.create_effect(move || {
            ...
            prev.set(Some(focus));
            let _ = cmd_tx.send(arvx_engine::EngineCommand::SetViewportFilter {
                id: PANEL_VIEWPORT,
                base_layers: arvx_engine::viewport::layer::BUILD_PREVIEW,
    ...
    // note further up).
    {
        ...
        __scope.create_effect(move || {
            ...
            let _ = cmd_tx.send(arvx_engine::EngineCommand::SetCamera {
                id: PANEL_VIEWPORT,
                position: eye,
                yaw: t.yaw,
                pitch: t.pitch,
    ...
    surface.set_event_handler(move |event| {
        use SurfaceEvent::*;
        use arvx_runtime::input::InputMouseButton;
...
fn render_bake_action(
    ...
    let on_click = move || {
        let _ = cmd_tx.get().send(arvx_engine::EngineCommand::BakeProceduralEntity {
...
fn render_resolution(
    ...
    let on_change: std::rc::Rc<dyn Fn(String)> = std::rc::Rc::new(move |v: String| {
        let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SetProceduralVoxelSize {
            tier: v,
        });
    });
    super::prop_controls::prop_select(
        ...
        arvx_engine::components::PROCEDURAL_VOXEL_TIERS,
        on_change,
    )
}
