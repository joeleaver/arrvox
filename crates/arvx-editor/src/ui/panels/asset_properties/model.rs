...
#[component]
fn ModelPropertiesForm(info: arvx_engine::ModelInfo) -> NodeHandle {
...
fn model_import_fields(
    ...
    source_path: String,
    profile: Option<arvx_engine::import_profile::ImportProfile>,
    cmd_tx: CmdSignal,
    ...
    let import_cb = move |field: &'static str| -> Rc<dyn Fn(String)> {
        Rc::new(move |v: String| {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::UpdateImportField {
                source_path: src.get(),
                field: field.into(),
                value: v,
            });
            ...
            div {
                ...
                if is_importing.get() {
                    ...
                    {prop_button(__scope, "Re-import", "#2d5a2d", Rc::new(move || {
                        let _ = cmd_tx.get().send(arvx_engine::EngineCommand::ReimportModel {
                            source_path: src.get(),
                        });
                    }))}
                }
...
fn render_import_progress(
    __scope: &mut rinch::core::dom::RenderScope,
    progress: Memo<Option<arvx_engine::snapshot::ImportProgressInfo>>,
) -> rinch::core::dom::NodeHandle {
    let store = use_context::<EditorStore>();
    rsx! {
        div {
            ...
            div {
                ...
                {|| {
                    ...
                    // path the `arvx-import` stage message embeds
