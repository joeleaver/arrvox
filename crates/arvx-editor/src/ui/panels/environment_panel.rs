...
pub fn EnvironmentPanel() -> NodeHandle {
    ...
    let env_f32 = move |field: &'static str| -> Rc<dyn Fn(f32)> {
        Rc::new(move |v: f32| {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::UpdateEnvironment {
                field: field.into(),
                value: v.to_string(),
            });
        })
    ...
    let apply_preset: Rc<dyn Fn(CloudQualityPreset)> = {
        Rc::new(move |preset: CloudQualityPreset| {
            ...
            let send = |field: &str, v: String| {
                let _ = tx.send(arvx_engine::EngineCommand::UpdateEnvironment {
                    field: field.into(),
                    value: v,
                });
            };
            ...
            if !sky_collapsed.get() {
                div {
                    ...
                    {prop_checkbox_memo(__scope, "Override Sky Top", sky_top_override_on, {
                        ...
                        Rc::new(move |v: bool| {
                            if !v {
                                let _ = tx.get().send(arvx_engine::EngineCommand::UpdateEnvironment {
