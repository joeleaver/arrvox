...
use arvx_engine::procedural_snapshot::{
...
fn render_params(
    ...
    _cmd_tx: Signal<crossbeam::channel::Sender<arvx_engine::EngineCommand>>,
) -> Node {
...
fn NodeParamsSection(node_info: ProceduralNodeInfo) -> NodeHandle {
    let store = use_context::<EditorStore>();
    let cmd_tx: Signal<crossbeam::channel::Sender<arvx_engine::EngineCommand>> =
    ...
    let on_remove: Option<Rc<dyn Fn()>> = if !is_root {
        Some(Rc::new(move || {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::RemoveProceduralNode {
                node_id,
            });
        }))
    } else {
    ...
    let pos_control = {
        let on_change: Rc<dyn Fn([f32; 3])> = Rc::new(move |val: [f32; 3]| {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SetProceduralNodePosition {
                node_id,
    ...
    let rot_control = {
        let on_change: Rc<dyn Fn([f32; 3])> = Rc::new(move |val: [f32; 3]| {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SetProceduralNodeRotation {
                node_id,
    ...
    let scale_control = {
        let on_change: Rc<dyn Fn([f32; 3])> = Rc::new(move |val: [f32; 3]| {
            let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SetProceduralNodeScale {
                node_id,
    ...
    match param.value {
        ProceduralParamValue::Float(v) => {
            ...
            let on_change: Rc<dyn Fn(f32)> = Rc::new(move |val: f32| {
                signal.set(val);
                let _ = cmd_tx.get().send(arvx_engine::EngineCommand::SetProceduralNodeParam {
                    node_id: node_id.get(),
