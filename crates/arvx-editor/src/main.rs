//! Arrvox Editor — thin client over the ArvxEngine.
//!
//! Creates an ArvxEngine on its own thread and a rinch UI as a thin client.
...
fn push_editor_layout(
    tx: &crossbeam::channel::Sender<arvx_engine::EngineCommand>,
    store: EditorStore,
) {
    let state = PersistedEditorState::capture(store);
    match serde_json::to_string(&state) {
        Ok(json) => {
            let _ = tx.send(arvx_engine::EngineCommand::SetEditorLayout { json });
        }
        Err(e) => eprintln!("[arvx-editor] failed to serialize editor layout: {e}"),
    }
}

/// Wrapper for the engine command sender, stored in rinch context.
#[derive(Clone)]
pub struct CommandSender(pub crossbeam::channel::Sender<arvx_engine::EngineCommand>);
...
fn build_menus(
    cmd_tx: crossbeam::channel::Sender<arvx_engine::EngineCommand>,
    store: EditorStore,
    ...
    let file_menu = Menu::new()
        .item(MenuItem::new("New Project").on_click({
            ...
            move || {
                if let Some(path) = rfd::FileDialog::new()
                    ...
                    .save_file()
                {
                    let _ = tx.send(arvx_engine::EngineCommand::NewProject {
        ...
        .item(MenuItem::new("Open Project...").on_click({
            ...
            move || {
                if let Some(path) = rfd::FileDialog::new()
                    ...
                    .pick_file()
                {
                    let _ = tx.send(arvx_engine::EngineCommand::OpenProject {
                        path: path.to_string_lossy().into_owned(),
                    });
                }
            }
        ...
        .item(MenuItem::new("Import Asset...").on_click({
            ...
            move || {
                if let Some(path) = rfd::FileDialog::new()
                    ...
                    .pick_file()
                {
                    let _ = tx.send(arvx_engine::EngineCommand::ImportAsset {
        ...
        .item(MenuItem::new("Save").shortcut("Ctrl+S").on_click({
            ...
            move || {
                ...
                let _ = tx.send(arvx_engine::EngineCommand::SaveScene { path: None });
            }
        }))
        .item(MenuItem::new("Save As...").shortcut("Ctrl+Shift+S").on_click({
            ...
            move || {
                if let Some(path) = rfd::FileDialog::new()
                    ...
                    .add_filter("Arrvox Scene", &["arvxscene"])
                    .save_file()
                {
                    push_editor_layout(&tx, store);
                    let _ = tx.send(arvx_engine::EngineCommand::SaveScene {
    ...
    for leaf in ["Sphere", "Box", "Capsule", "Cylinder", "Torus", "Ramp"] {
        spawn_menu = spawn_menu.item(MenuItem::new(leaf).on_click({
            let tx = tx.clone();
            move || {
                let _ = tx.send(arvx_engine::EngineCommand::SpawnProceduralObject {
                    name: leaf.to_string(),
    ...
    spawn_menu = spawn_menu
        ...
        .item(MenuItem::new("Point Light").on_click({
            let tx = tx.clone();
            move || { let _ = tx.send(arvx_engine::EngineCommand::SpawnPointLight); }
        }))
        .item(MenuItem::new("Spot Light").on_click({
            let tx = tx.clone();
            move || { let _ = tx.send(arvx_engine::EngineCommand::SpawnSpotLight); }
        }))
        .separator()
    ...
    let edit_menu = Menu::new()
        ...
        .item(MenuItem::new("Duplicate").shortcut("Ctrl+D").on_click({
            let tx = tx.clone();
            move || {
                let _ = tx.send(arvx_engine::EngineCommand::DuplicateSelected);
            }
        }))
        .item(MenuItem::new("Delete").shortcut("Delete").on_click({
            let tx = tx.clone();
            move || {
                let _ = tx.send(arvx_engine::EngineCommand::DeleteSelected);
            }
        }));
    ...
    let view_menu = Menu::new()
        .item(MenuItem::new("Show Colliders").on_click({
            ...
            move || {
                ...
                toggle.set(new_val);
                let _ = tx.send(arvx_engine::EngineCommand::SetViewOption {
...
fn main() -> anyhow::Result<()> {
    ...
    // Optional positional CLI arg: path to a `.arvxproject` to auto-open
    ...
    // 3. Start the engine.
    let engine = arvx_engine::ArvxEngine::spawn(
        ...
        // State callback: push engine state to EditorStore signals (cross-thread).
        {
            ...
            struct ProfilingState {
                ...
                smoothed_cpu: arvx_engine::profiling::CpuPhaseTimings,
            ...
            let prof_state = std::sync::Arc::new(std::sync::Mutex::new(ProfilingState {
                ...
                smoothed_cpu: arvx_engine::profiling::CpuPhaseTimings::default(),
            ...
            Box::new(move |update: &arvx_engine::StateUpdate| {
    ...
    if let Some(ref path) = auto_open_project {
        if std::path::Path::new(path).exists() {
            eprintln!("[arvx-editor] auto-opening project: {path}");
            let _ = engine.cmd_tx.send(arvx_engine::EngineCommand::OpenProject {
                path: path.clone(),
            });
        } else {
            eprintln!(
                "[arvx-editor] project path does not exist, skipping auto-open: {path}"
            );
        }
    }
