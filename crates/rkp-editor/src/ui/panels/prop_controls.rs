...
const SLIDER_STYLE: &str = "flex:1;height:4px;accent-color:#4fc3f7;cursor:pointer;";
...
/// Numeric text input for unbounded f64 values.
...
/// Color swatch + native picker. Works with [f32; 4] RGBA (alpha always 1.0).
pub fn prop_color(
    __scope: &mut Scope,
    label: &str,
    value: Signal<[f32; 4]>,
    ...
    let label = label.to_string();
    let hex = Signal::new(rgba_to_hex(value.get()));

    rsx! {
        div {
            style: ROW_STYLE,
            div { style: LABEL_STYLE, {label} }
            // Color swatch (reactive)
            div {
                style: {move || format!(
                    "width:22px;height:22px;border-radius:3px;border:1px solid #3c3c3c;\
                     background:{};flex-shrink:0;cursor:pointer;",
                    hex.get()
                )},
            }
            // Native color picker
            ...
            // Hex display
            div {
                style: "font-size:10px;color:#777;font-family:monospace;flex-shrink:0;",
                {move || hex.get()}
            }
        }
    }
