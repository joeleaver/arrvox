...
//! hierarchy), `pool_size` (brick pool in arvx-core is sized
...
/// Configuration for a mesh-to-.arvx import.
///
/// Constructed programmatically or from an editor-level import profile
/// (`arvx-engine::import_profile`). All fields have sensible defaults
...
/// Result of a successful mesh-to-.arvx import.
#[derive(Debug, Clone)]
pub struct ImportResult {
    ...
    /// Output `.arvx` file size in bytes.
    pub file_size: u64,
    /// Path to the generated `.arvxskel` skeleton asset, if the source had bones.
