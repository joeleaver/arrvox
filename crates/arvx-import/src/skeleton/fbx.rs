...
use arvx_animation::skeleton::{Bone, Skeleton};
    ...
    for skin in scene.skin_deformers.iter() {
        for cluster in skin.clusters.iter() {
            let Some(ref bone_node) = cluster.bone_node else {
                eprintln!("[arvx-import] warn: FBX skin cluster has no bone node, skipping");
                continue;
            };
...
fn get_vertex_skin_weights(
    ...
    if vertex_idx >= skin.vertices.len() {
        eprintln!(
            "[arvx-import] warn: FBX vertex_idx {vertex_idx} out of skin.vertices ({}) for mesh '{}'",
    ...
    if begin + n > skin.weights.len() {
        eprintln!(
            "[arvx-import] warn: FBX vertex {vertex_idx} weight range [{begin}, {}) exceeds skin.weights ({})",
    ...
    if dropped_clusters > 0 {
        eprintln!(
            "[arvx-import] warn: FBX vertex {vertex_idx} had {dropped_clusters} cluster indices with no bone (bone_count = {bone_count})"
        );
    }
    if nan_weights > 0 {
        eprintln!(
            "[arvx-import] warn: FBX vertex {vertex_idx} had {nan_weights} non-finite weights"
        );
    }
...
fn extract_animations(
    ...
    for stack in &scene.anim_stacks {
        ...
        let baked = match ufbx::bake_anim(scene, &stack.anim, bake_opts) {
            ...
            Err(e) => {
                eprintln!(
                    "[arvx-import] warn: failed to bake FBX animation '{name}': {}",
                    e.description
                );
                continue;
            }
