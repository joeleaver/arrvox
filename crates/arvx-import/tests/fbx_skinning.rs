...
//! the `ARVX_TEST_FBX` environment variable to a path containing a
//! rigged FBX (e.g. any Mixamo character):
//!
//! ```bash
//! ARVX_TEST_FBX=/path/to/Walking.fbx cargo test -p arvx-import --test fbx_skinning
//! ```
//!
...
use arvx_import::skeleton::extract_skeleton;

fn test_fbx_path() -> Option<std::path::PathBuf> {
    std::env::var("ARVX_TEST_FBX").ok().map(|s| s.into())
}
...
fn extract_skeleton_has_valid_skinning() {
    let Some(path) = test_fbx_path() else {
        eprintln!("ARVX_TEST_FBX unset — skipping");
        return;
    };
...
fn import_at_smallest_voxel_tier_does_not_crash() {
    ...
    let output = tmp.path().join("walking_fine.arvx");
...
fn multi_skin_unified_bone_table() {
    ...
    if scene.skin_deformers.len() < 2 {
        eprintln!("skipping: ARVX_TEST_FBX only has {} skin deformer(s)", scene.skin_deformers.len());
        return;
    }
...
/// Full round-trip import → .arvx + .arvxskel on disk. Verifies the
...
fn import_fbx_writes_rkp_and_rkskel() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let output = tmp.path().join("walking.arvx");
    ...
    assert!(output.exists(), ".arvx file should exist on disk");
    let size = std::fs::metadata(&output).unwrap().len();
    assert_eq!(size, result.file_size, "ImportResult.file_size should match");
    assert!(size > 1024, ".arvx file suspiciously small: {size} bytes");

    let skel_path = result.skeleton_path.expect(".arvxskel should be written for a skinned FBX");
    assert!(skel_path.exists(), ".arvxskel file should exist on disk");
    let skel_size = std::fs::metadata(&skel_path).unwrap().len();
    assert!(skel_size > 128, ".arvxskel file suspiciously small: {skel_size} bytes");
