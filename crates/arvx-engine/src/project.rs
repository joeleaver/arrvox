//! Project file format — .arvxproject files.
//!
//! A project is a directory containing a `.arvxproject` JSON file,
//! a `scenes/` directory with `.arvxscene` files, and an `assets/`
//! directory with `.arvx` models and materials.

use serde::{Deserialize, Serialize};
use std::path::{Path, PathBuf};

/// Project descriptor — serialized to `.arvxproject` as JSON.
...
///
/// `path` should be the desired `.arvxproject` file path. The project
/// is created inside a new subdirectory named after the project:
/// e.g. `/home/user/MyProject.arvxproject` → `/home/user/MyProject/MyProject.arvxproject`
///
...
pub fn create_project(path: &Path) -> Result<PathBuf, String> {
    ...
    crate::scene_io::save_scene(&scene, &project_dir.join("scenes/default.arvxscene"))?;

    eprintln!("[ArvxEngine] created project '{}' at {}", project_name, project_dir.display());
    Ok(project_dir.to_path_buf())
}

/// Load a project from a `.arvxproject` file.
