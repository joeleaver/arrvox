...
pub struct EnvironmentSettings {
    ...
    /// `ARVX_CSM_NEAR=...` for headless tuning.
...
mod atmo {
    // Must match constants in arvx_shade.wgsl (Bruneton 2017 / Hillaire 2020).
...
impl EnvironmentSettings {
    ...
    /// Multi-scattering boost — must match MULTI_SCATTER_BOOST in arvx_shade.wgsl.
    pub fn to_shade_params(&self, cam_y: f32) -> arvx_render::arvx_shade::ShadeParams {
        let d = self.sun_direction();
        let sun_toward = [-d[0], -d[1], -d[2]];
        ...
        arvx_render::arvx_shade::ShadeParams {
            num_lights: 1,
    ...
    pub fn to_volumetric_params(
        &self,
        cam: &arvx_render::arvx_scene::CameraUniforms,
        width: u32,
        height: u32,
        frame_index: u32,
    ) -> arvx_render::arvx_volumetric::VolumetricParams {
        let d = self.sun_direction();
    ...
    pub fn to_cloud_params(&self, time: f32) -> arvx_render::arvx_volumetric::CloudParams {
        let wind_rad = self.cloud_wind_dir.to_radians();
        arvx_render::arvx_volumetric::CloudParams {
            altitude: [
