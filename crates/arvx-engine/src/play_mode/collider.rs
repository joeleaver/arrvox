...
use arvx_physics::rapier_world::to_rapier_vec3;
...
fn brick_cell(pool: &[u32], brick_id: u32, x: u32, y: u32, z: u32) -> u32 {
    let offset = brick_id as usize * arvx_core::BRICK_CELLS as usize
        + (x
            + y * arvx_core::BRICK_DIM
            + z * arvx_core::BRICK_DIM * arvx_core::BRICK_DIM) as usize;
    pool.get(offset).copied().unwrap_or(arvx_core::BRICK_EMPTY)
}
...
pub fn build_coarse_collider(
    ...
    fn walk(
        ...
        use arvx_core::{BRICK_DIM, BRICK_EMPTY};
...
pub fn compute_tight_local_aabb(
    ...
    Some(arvx_core::Aabb { min: lo, max: hi })
}
