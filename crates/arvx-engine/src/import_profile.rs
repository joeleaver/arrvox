//! Import profile — per-asset import settings stored as `.arvximport` sidecar files.
//!
//! When a mesh file (e.g. `bunny.glb`) is imported, its settings are stored
//! alongside it as `bunny.glb.arvximport`. This lets the user tweak resolution,
//! rotation, naming, etc. and re-import without losing their configuration.

use std::path::{Path, PathBuf};

use arvx_import::ImportConfig;
use serde::{Deserialize, Serialize};

/// Per-asset import profile — serialized to `.arvximport` JSON sidecar.
...
impl ImportProfile {
    /// Convert to the arvx-import ImportConfig used by the import pipeline.
    ...
    /// Sidecar path for a source mesh file (e.g. `bunny.glb` → `bunny.glb.arvximport`).
