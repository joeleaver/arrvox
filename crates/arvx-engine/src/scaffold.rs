...
//! This module generates a complete Rust cdylib crate in `.arvx-cache/gameplay/`
...
pub const CACHE_DIR: &str = ".arvx-cache";
...
fn gen_cargo_toml(engine_path: &str, macros_path: &str) -> String {
    ...
    let procedural_path = engine_root.join("crates/arvx-procedural");
    format!(
        r#"[package]
name = "gameplay"
version = "0.1.0"
...
[dependencies]
arvx-engine = {{ path = "{engine_path}" }}
arvx-macros = {{ path = "{macros_path}" }}
arvx-procedural = {{ path = "{procedural}" }}
inventory = "0.3"
...
fn gen_lib_rs(components: &[String], systems: &[String], generators: &[String]) -> String {
    ...
    s.push_str("use arvx_engine::component_registry::ComponentEntry;\n");
    s.push_str("use arvx_engine::behavior::SystemEntry;\n");
    s.push_str("use arvx_engine::generator::GeneratorEntry;\n\n");

    // Component FFI export.
    s.push_str("#[unsafe(no_mangle)]\n");
    s.push_str("pub extern \"C\" fn arvx_gameplay_entries() -> arvx_engine::gameplay_loader::GameplayEntries {\n");
    s.push_str("    let entries: Vec<&'static ComponentEntry> =\n");
    s.push_str("        inventory::iter::<ComponentEntry>.into_iter().collect();\n");
    s.push_str("    arvx_engine::gameplay_loader::GameplayEntries::from_iter(entries)\n");
    s.push_str("}\n\n");

    // System FFI export.
    s.push_str("#[unsafe(no_mangle)]\n");
    s.push_str("pub extern \"C\" fn arvx_gameplay_systems() -> arvx_engine::gameplay_loader::GameplaySystems {\n");
    s.push_str("    let entries: Vec<&'static SystemEntry> =\n");
    s.push_str("        inventory::iter::<SystemEntry>.into_iter().collect();\n");
    s.push_str("    arvx_engine::gameplay_loader::GameplaySystems::from_iter(entries)\n");
    s.push_str("}\n\n");

    // Generator FFI export.
    s.push_str("#[unsafe(no_mangle)]\n");
    s.push_str("pub extern \"C\" fn arvx_gameplay_generators() -> arvx_engine::gameplay_loader::GameplayGenerators {\n");
    s.push_str("    let entries: Vec<&'static GeneratorEntry> =\n");
    s.push_str("        inventory::iter::<GeneratorEntry>.into_iter().collect();\n");
    s.push_str("    arvx_engine::gameplay_loader::GameplayGenerators::from_iter(entries)\n");
    s.push_str("}\n");
...
mod tests {
    ...
    fn scaffold_from_scripts() {
        ...
        assert!(lib_rs.contains("arvx_gameplay_systems"));
        // Always emits the generator FFI export, even if no generators found.
        assert!(lib_rs.contains("arvx_gameplay_generators"));
