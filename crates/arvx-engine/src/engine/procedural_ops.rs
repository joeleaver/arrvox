...
impl EngineState {
    ...
    pub(crate) fn reload_user_shaders(&mut self) -> Option<()> {
        use arvx_render::shader_composer;
    ...
    /// `{scene_dir}/{scene_stem}.bakes/{uuid}.arvx`. Returns `None` when
    /// the scene has no on-disk path yet (unsaved scratch session) or
    /// the entity has no UUID. The relative form
    /// (`{scene_stem}.bakes/{uuid}.arvx`) is what `SceneObject.procedural_cache`
    ...
    pub(crate) fn enqueue_bake(&mut self, entity: hecs::Entity) -> Option<u64> {
        ...
        // Build the sidecar .arvx path: `{scene_dir}/{scene_stem}.bakes/{uuid}.arvx`.
    ...
    pub(crate) fn apply_proxy_mesh_result(
        ...
        // PERF_DEBT B1+C2: this entity's ArvxGpuInstance row needs
...
pub(crate) fn procedural_voxel_params(tree: &arvx_procedural::ProceduralObject, base_voxel_size: f32) -> (arvx_core::Aabb, f32) {
    let tight = arvx_procedural::compute_bounds(tree);
