...
pub(crate) fn collect_ghost_primitives(tree: &arvx_procedural::ProceduralObject) -> Vec<u32> {
    let mut out = Vec::new();
...
pub(crate) fn raycast_leaf_primitive(
    ...
    leaf_id: arvx_procedural::NodeId,
    ...
    for _ in 0..MAX_STEPS {
        let p = ro + rd * t;
        let d = arvx_procedural::eval_leaf_distance(p, &node.kind);
...
pub(crate) fn find_path(
    tree: &arvx_procedural::ProceduralObject,
    start: arvx_procedural::NodeId,
    target: arvx_procedural::NodeId,
    out_path: &mut Vec<arvx_procedural::NodeId>,
) -> bool {
    out_path.push(start);
    if start == target {
        return true;
