...
impl EngineState {
    pub(crate) fn update_gizmo(&mut self) {
        ...
        let left_pressed = self.input_system.raw_state().is_mouse_button_pressed(arvx_runtime::input::InputMouseButton::Left);
            ...
            match self.gizmo.mode {
                crate::gizmo::GizmoMode::Translate => {
                    ...
                    if let Ok(mut t) = self.world.get::<&mut crate::components::Transform>(selected) {
                        ...
                        self.gpu_objects_dirty.mark_entity_transform(selected);
                    }
                }
    ...
    pub(crate) fn build_bone_wireframes(&self) -> Vec<arvx_render::LineVertex> {
        ...
        let Some((transform, skeleton)) = query.get() else { return verts };
        {
            ...
            for (i, &parent) in bones.hierarchy.iter().enumerate() {
                ...
                if parent >= 0 && (parent as usize) < bones.bones.len() {
                    ...
                    verts.push(arvx_render::LineVertex {
                    ...
                    // with no child.
                    verts.extend(arvx_render::wireframe::crosshair(child, 0.05, color));
                }
            }
        }
        verts
    ...
    pub(crate) fn build_gizmo_wireframe(&self) -> Vec<arvx_render::LineVertex> {
        ...
        for (entity, (transform, pl)) in self.world.query::<(&crate::components::Transform, &crate::components::PointLight)>().iter() {
            ...
            verts.extend(arvx_render::wireframe::crosshair(transform.position, 0.2, icon_color));
            // Range sphere only when selected.
            if selected {
                verts.extend(arvx_render::wireframe::point_light_wireframe(
        ...
        for (entity, (transform, sl)) in self.world.query::<(&crate::components::Transform, &crate::components::SpotLight)>().iter() {
            ...
            if selected {
                verts.extend(arvx_render::wireframe::spot_light_wireframe(
        ...
        // `arvx_shade.wgsl`'s brush overlay block) — the ring is
    ...
    pub(crate) fn rebuild_collider_cache_for(&mut self, entity: hecs::Entity) {
        ...
        let (resolved_shape, voxel_coords, voxel_size) = match rb.collider_shape {
            arvx_physics::rigid_body::ColliderShape::Auto => {
                if let Some(ref sp) = spatial {
                    ...
                    if coords.is_empty() {
                        (arvx_physics::rigid_body::ColliderShape::Box, Vec::new(), 0.0)
                    } else {
                        (arvx_physics::rigid_body::ColliderShape::Auto, coords, cell_size)
                    }
                } else {
                    (arvx_physics::rigid_body::ColliderShape::Box, Vec::new(), 0.0)
                }
    ...
    pub(crate) fn build_collider_wireframes(&self) -> Vec<arvx_render::LineVertex> {
        use arvx_physics::rigid_body::{BodyType, ColliderShape};
        let mut verts = Vec::new();
        ...
        for (_entity, (transform, rb, cache)) in self.world.query::<(
            ...
            match cache.shape {
                ...
                ColliderShape::Sphere => {
                    ...
                    verts.extend(arvx_render::wireframe::sphere_wireframe(center, r, color));
                }
                ColliderShape::Capsule => {
                    ...
                    verts.extend(arvx_render::wireframe::sphere_wireframe(top, r, color));
                    verts.extend(arvx_render::wireframe::sphere_wireframe(bot, r, color));
                    for angle in [0.0f32, std::f32::consts::FRAC_PI_2, std::f32::consts::PI, 3.0 * std::f32::consts::FRAC_PI_2] {
                        ...
                        verts.push(arvx_render::LineVertex { position: (top + offset).to_array(), color });
                        verts.push(arvx_render::LineVertex { position: (bot + offset).to_array(), color });
                    }
                }
                ColliderShape::Auto => {
                        ...
                        && cache.voxel_coords.len() <= MAX_WIRE_CELLS
                    {
                        ...
                        for coord in &cache.voxel_coords {
                            ...
                            verts.extend(arvx_render::wireframe::aabb_wireframe(world_min, world_max, color));
                        }
                    } else {
