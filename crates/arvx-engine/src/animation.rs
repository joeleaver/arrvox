...
//!
//! 1. [`AnimationAssetCache`] — path-keyed cache of loaded `.arvxskel`
...
use arvx_animation::skeleton_asset::{load_rkskel, SkeletonAsset};

use crate::components::{AnimationPlayer, Skeleton};

/// Path-keyed cache of loaded [`SkeletonAsset`]s.
///
/// `.arvxskel` files are small RON; instances of the same asset share an
...
impl AnimationAssetCache {
    ...
    /// Load the `.arvxskel` at `path` (if not already cached). Returns the
...
/// Build a [`Skeleton`] component for an entity that's just loaded `path.arvxskel`.
///
...
/// duration. Duplicated from `arvx_animation::AnimationPlayer::advance`
...
pub fn tick(world: &mut hecs::World, dt: f32) -> bool {
    ...
    for (_entity, (skel, player)) in world.query_mut::<(&mut Skeleton, &mut AnimationPlayer)>() {
        ...
        // data lives in arvx-import's normalised frame. Conjugate by
...
mod tests {
    use super::*;
    use glam::{Quat, Vec3};
    use arvx_animation::clip::{AnimationClip, BoneChannel, Keyframe};
    use arvx_animation::skeleton::{Bone, Skeleton as SkelData};
    use arvx_animation::skeleton_asset::SkeletonAsset;
    ...
    fn tick_updates_current_pose_for_active_clip() {
        ...
        let entity = world.spawn((
            skeleton_component(asset.clone(), PathBuf::from("test.arvxskel"), glam::Vec3::ZERO),
            AnimationPlayer {
                clip_name: "wave".into(),
                time: 0.0,
                speed: 1.0,
