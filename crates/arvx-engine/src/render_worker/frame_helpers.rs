...
pub(super) fn compute_tlas_scene_aabb(
    instances: &[arvx_render::arvx_gpu_object::ArvxGpuInstance],
    assets: &[arvx_render::arvx_gpu_object::ArvxGpuAsset],
) -> ([f32; 3], [f32; 3]) {
...
pub(super) fn prepare_shadow_maps(
    ...
    // env-var override (`ARVX_CSM_THRESHOLD_FALLOFF`) takes precedence
    ...
    use arvx_render::shadow_map_pass::{
    ...
    // (ARVX_CSM_*) take precedence for headless / CI testing.
    let csm_max_distance = std::env::var("ARVX_CSM_MAX_DISTANCE")
        .ok()
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(frame.shadow_csm_max_distance)
        .clamp(10.0, 1000.0);
    let csm_lambda = std::env::var("ARVX_CSM_LAMBDA")
        .ok()
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(frame.shadow_csm_lambda)
        .clamp(0.0, 1.0);
    let csm_near = std::env::var("ARVX_CSM_NEAR")
        .ok()
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(frame.shadow_csm_near)
        .clamp(0.05, 10.0);
    let depth_bias = frame.shadow_csm_depth_bias.max(0.0);
    let sharp_distance = std::env::var("ARVX_CSM_SHARP_DISTANCE")
        .ok()
        .and_then(|s| s.parse::<f32>().ok())
        .unwrap_or(frame.shadow_csm_sharp_distance)
        .clamp(0.0, 100.0);
    let map_size = std::env::var("ARVX_CSM_MAP_SIZE")
        .ok()
    ...
    // ARVX_SHADOW_FIT_LOG with the scene-AABB context.
    let fit_log = std::env::var("ARVX_CSM_FIT_LOG").is_ok()
        || std::env::var("ARVX_SHADOW_FIT_LOG").is_ok();
    if fit_log {
