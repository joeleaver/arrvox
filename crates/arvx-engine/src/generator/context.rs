...
use arvx_core::{Aabb, BakeArtifact, WorldPosition};
...
pub struct GeneratorContext<'w> {
    ...
    #[allow(dead_code)]
    scene_mgr: Option<&'w Arc<Mutex<ArvxSceneManager>>>,
}
...
impl<'w> GeneratorContext<'w> {
    ...
    pub(crate) fn new_worker(
        ...
        scene_mgr: &'w Arc<Mutex<ArvxSceneManager>>,
    ...
    pub fn emit_child(
        ...
        tree: &arvx_procedural::ProceduralObject,
        voxel_size: f32,
        ...
        let aabb = arvx_procedural::compute_bounds(tree);
        let instructions = arvx_procedural::flatten_tree(tree);
...
mod tests {
    ...
    fn emit_child_without_worker_context_errors() {
        let mut c = ctx();
        let obj = arvx_procedural::ProceduralObject::new(
            arvx_procedural::NodeKind::Root,
        );
