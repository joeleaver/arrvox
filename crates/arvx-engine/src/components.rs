//! Built-in ECS components for the Arrvox engine.
//!
...
pub struct Renderable {
    ...
    /// Handle into the scene manager's asset cache for .arvx-backed
    ...
    #[serde(skip)]
    pub asset_handle: Option<arvx_render::AssetHandle>,
...
pub enum RenderGeometry {
    ...
    /// allocated via `ArvxSceneManager::reserve_procedural_handle`.
    ProxyMesh(ProxyMeshData),
}

/// Renderer state for a proxy-mesh procedural. The actual triangle
/// data lives on the GPU in `ArvxRenderer`'s `proxy_mesh_buffers`
/// keyed by `handle`.
#[derive(Debug, Clone)]
pub struct ProxyMeshData {
    /// Renderer mesh handle. Allocated from the same handle space
    /// as disk assets so `proxy_mesh_buffers[handle.raw()]` is the
    /// GPU buffer pair. Released via
    /// `ArvxSceneManager::release_procedural_handle` when the entity
    /// is destroyed or re-baked into a different mode.
    pub handle: arvx_render::AssetHandle,
...
impl RenderGeometry {
    ...
    #[inline]
    pub fn renderer_handle(&self) -> Option<arvx_render::AssetHandle> {
        match self {
            RenderGeometry::Octree(_) => None,
    ...
    #[inline]
    pub fn aabb(&self) -> arvx_core::Aabb {
        match self {
...
// retired alongside the splat-march path. Imported `.arvx` assets
// still voxelize for sculpt/paint compatibility (that path lives
// in `arvx-import` / `BakeInput::Artifact`, not here).
...
pub struct SpatialData {
    ...
    /// Brick ids owned by this procedural allocation (empty for .arvx-backed
...
pub struct RigidBody {
    ...
    pub body_type: arvx_physics::rigid_body::BodyType,
    /// Collision shape. Auto = voxel collider for voxelized, box for analytical.
    pub collider_shape: arvx_physics::rigid_body::ColliderShape,
...
impl Default for RigidBody {
    fn default() -> Self {
        Self {
            body_type: arvx_physics::rigid_body::BodyType::Dynamic,
            collider_shape: arvx_physics::rigid_body::ColliderShape::Auto,
            mass: 1.0,
            friction: 0.5,
            restitution: 0.3,
...
pub struct ColliderCache {
    ...
    pub shape: arvx_physics::rigid_body::ColliderShape,
...
pub struct ProceduralGeometry {
    ...
    pub tree: arvx_procedural::ProceduralObject,
...
impl ProceduralGeometry {
    ...
    pub fn default_sphere() -> Self {
        ...
        Self::with_leaf(NodeKind::Sphere(
            arvx_procedural::node_kind::SphereParams::default(),
        ))
    }

    /// Create a procedural object with a single-child Root containing `leaf`.
    pub fn with_leaf(leaf: arvx_procedural::NodeKind) -> Self {
        use arvx_procedural::*;
...
///
/// Attached automatically by the engine when a `.arvx` load finds a sibling
/// `.arvxskel`. Not serialized through the scene file — rebuilt on load from
...
/// that arvx-import applied to the voxel data but *not* to the
/// skeleton. Bone matrices from `Skeleton::evaluate` are in glTF
/// space; voxel data is in normalised "arvx-local" space. The pose
...
#[derive(Clone)]
pub struct Skeleton {
    pub asset: std::sync::Arc<arvx_animation::skeleton_asset::SkeletonAsset>,
...
impl Skeleton {
    ...
    pub fn compute_bind_world_origins(
        asset: &arvx_animation::skeleton_asset::SkeletonAsset,
    ) -> Vec<glam::Vec3> {
    ...
    /// Build the glTF→arvx-voxel-space transform from the asset's
    /// normalization parameters. Mirrors the mesh preparation that
    /// `arvx-import::normalize::prepare_mesh` applies:
...
pub struct AnimationPlayer {
    ...
    pub loop_mode: arvx_animation::player::LoopMode,
...
impl Default for AnimationPlayer {
    fn default() -> Self {
        Self {
            ...
            speed: 1.0,
            playing: false,
            loop_mode: arvx_animation::player::LoopMode::Loop,
            forward: true,
        }
    }
}
