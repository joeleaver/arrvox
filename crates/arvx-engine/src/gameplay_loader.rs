...
//! 3. Unload old dylib
//! 4. Load new dylib
//! 5. Call `arvx_gameplay_entries()` to discover new component entries
...
impl GameplayLoader {
    ...
    pub fn load(&mut self, path: &Path) -> Result<&[&'static ComponentEntry], String> {
        ...
        let entries_fn: libloading::Symbol<EntryFn> = unsafe {
            lib.get(b"arvx_gameplay_entries")
        ...
        let systems: Vec<&'static SystemEntry> = unsafe {
            if let Ok(systems_fn) = lib.get::<SystemsFn>(b"arvx_gameplay_systems") {
        ...
        let generators: Vec<&'static GeneratorEntry> = unsafe {
            if let Ok(generators_fn) = lib.get::<GeneratorsFn>(b"arvx_gameplay_generators") {
