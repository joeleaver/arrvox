...
//! thread owns wgpu (`device`, `queue`, [`ArvxRenderer`], the per-
...
use arvx_core::OverlayEntry;
use arvx_render::{
    arvx_atmosphere::AtmosphereFrameParams,
    arvx_god_rays::GodRayParams,
    arvx_gpu_object::{ArvxGpuAsset, ArvxGpuInstance},
    arvx_grid::GridParams,
...
pub struct RenderFrame {
    ...
    pub gpu_assets: Arc<Vec<ArvxGpuAsset>>,
    ...
    pub gpu_instances: Arc<Vec<ArvxGpuInstance>>,
    /// Flat per-instance paint overlay entries. Each `ArvxGpuInstance`
    ...
    /// `ArvxGpuInstance` uses its `sculpt_offset` + `sculpt_count` to
    ...
    /// references a loaded `.arvx` asset. Procedural objects without
    /// an `AssetHandle` are not represented here (procedurals ride
    /// `proxy_draws` instead).
    pub mesh_draws: Arc<Vec<arvx_render::mesh_instance::MeshDraw>>,
    ...
    pub proxy_draws: Arc<Vec<arvx_render::mesh_proxy_pass::ProxyDraw>>,
    ...
    pub user_shader_infos: Vec<arvx_render::shader_composer::UserShaderInfo>,
    ...
    /// registered.
    pub user_shader_entries: Arc<Vec<arvx_render::shader_composer::UserShaderEntry>>,
    ...
    pub bone_matrix_lbs_dirty: arvx_core::DirtyRanges,
    /// Same as above for `bone_matrix_dqs`.
    pub bone_matrix_dqs_dirty: arvx_core::DirtyRanges,
    ...
    pub gpu_instance_overlays_dirty: arvx_core::DirtyRanges,
    /// PERF_DEBT.md D3: same shape as
    /// [`Self::gpu_instance_overlays_dirty`] but for the
    /// `instance_sculpt_buffer` upload.
    pub gpu_instance_sculpts_dirty: arvx_core::DirtyRanges,
...
pub struct RenderProcRaymarch {
    pub instructions: Vec<arvx_procedural::ProcInstruction>,
    ...
    pub ghost_instructions: Vec<arvx_procedural::ProcInstruction>,
...
pub struct RenderInit {
    ...
    pub scene_mgr: Arc<std::sync::Mutex<arvx_render::ArvxSceneManager>>,
