...
//! (ECS world), renderer (ArvxRenderer), physics, and animation. Communication
...
pub use command::{
    DragPreviewSource, EngineCommand, PaintMode, RegionShapeSpec, SculptMode, StampKindSpec,
};
...
pub use arvx_core::sculpt::FalloffCurve;
pub use snapshot::{StateUpdate, SceneObjectInfo, ModelInfo};
pub use material_library::MaterialInfo;
pub use engine::ArvxEngine;

// Re-export the proc macros for use by gameplay crates.
pub use arvx_macros::arvx_component;
pub use arvx_macros::arvx_generator;
pub use arvx_macros::arvx_system;
