...
use glam::Vec3;
use arvx_runtime::input::{InputKeyCode, InputMouseButton};
use uuid::Uuid;
...
pub enum EngineCommand {
    ...
    /// Spawn a generator entity from a `.arvxgen` preset on disk. The
    ...
    SpawnGeneratorPreset {
        /// Absolute path to the `.arvxgen` file (round-tripped from
    ...
    /// Drop a `.arvxgen` preset at a viewport pixel. Drag-and-drop
    ...
    /// Load an .arvx asset directly into the scene.
    LoadAsset {
        path: String,
        position: Vec3,
    },

    /// Drop an .arvx asset at a viewport pixel. The engine issues a
    ...
    /// Open an existing project (.arvxproject file).
    OpenProject {
        path: String,
    },
...
pub enum DragPreviewSource {
    ...
    /// `.arvxgen` preset on disk — loads a generator + overrides.
