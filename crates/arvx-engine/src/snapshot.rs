...
/// engine from the raw `ImportEvent` stream emitted by `arvx-import`.
...
pub struct StateUpdate {
    ...
    /// `assets/bunny.obj` instead of `/home/joe/dev/arrvox/assets/bunny.obj`.
    ...
    /// Generator presets discovered in `assets/generators/*.arvxgen`.
    ...
    /// Editor layout blob round-tripped from `.arvxproject`. Sent once on
    ...
    /// `assets/shaders/*.wgsl`).
    pub user_shaders: Option<Vec<arvx_render::shader_composer::UserShaderInfo>>,
...
pub struct ModelInfo {
    ...
    pub name: String,
    /// Full path to the .arvx file.
    pub path: String,
    /// Source mesh path (.glb/.obj/.fbx) if this was auto-imported.
    pub source_path: String,
    /// File size in bytes.
    pub size: u64,
    /// Total shell voxel count read from the .arvx header. Displayed
