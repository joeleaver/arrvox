...
//!
//! Tests stand up `.arvxmat` files on disk and load them via
//! `MaterialLibrary::scan()` — the same path the editor uses at
//! project load.

use arvx_engine::material_library::MaterialLibrary;
...
"#,
  ...
  "shader_params": { "density": 12.0, "wind_amp": 0.7 }
    ...
    let reg = arvx_render::shader_composer::scan_dir(&shaders_dir).unwrap();
    let mut lib = MaterialLibrary::new();
