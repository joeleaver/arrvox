...
use rkp_render::instance_march_pass::compose_march_main_source;
...
fn example_grass_shader_composes_and_validates() {
    ...
    // The march pipeline also splices grass's inst_to_local / inst_aabb
    // hooks. Failing here means the per-shader pool-read wrappers
    // generated by the composer don't compile against the march
    // template's `Aabb` struct + module-scope bindings.
    let march_source = compose_march_main_source(&chunks.inst_to_local, &chunks.inst_aabb);
    assert_wgsl_valid(&march_source, "instance_march_main+grass");
    assert!(
        march_source.contains("rkp_user_") && march_source.contains("_inst_to_local_at"),
        "compose.inst_to_local missing per-shader pool-read wrapper for grass",
    );
    assert!(
        march_source.contains("rkp_user_") && march_source.contains("_inst_aabb_at"),
        "compose.inst_aabb missing per-shader pool-read wrapper for grass",
    );
}
