...
use super::cache::FILL_TASK_BUCKET_MAX;
use super::overflow::{OverflowReadback, OVERFLOW_BUFFER_BYTES, OVERFLOW_COUNTER_COUNT};
...
pub(super) const PER_LEVEL_QUEUE_CAP: u32 = 2097152;
...
pub(super) const LEVEL_UNIFORM_STRIDE: u64 = 256;
...
/// overflow counters). Construction lives in `setup.rs`; the
/// per-frame work (`dispatch_regions`) lives in this file.
pub struct UserShaderPass {
    pub(super) group0_layout: wgpu::BindGroupLayout,
    pub(super) group1_layout: wgpu::BindGroupLayout,
    pub(super) group2_layout: wgpu::BindGroupLayout,
    pub(super) pipeline_layout: wgpu::PipelineLayout,
    pub(super) classify_pipeline: wgpu::ComputePipeline,
    pub(super) fill_pipeline: wgpu::ComputePipeline,
    ...
    pub(super) octree_alloc_buffer: wgpu::Buffer,
    pub(super) brick_alloc_buffer: wgpu::Buffer,
    pub(super) leaf_attr_alloc_buffer: wgpu::Buffer,
    pub(super) fill_task_alloc_buffer: wgpu::Buffer,
    ...
    pub(super) active_queue_buffer: wgpu::Buffer,
    pub(super) active_count_buffer: wgpu::Buffer,
    ...
    pub(super) fill_task_pool_buffer: wgpu::Buffer,

    /// Overflow counters — async readback ring on the CPU side.
    pub(super) overflow_buffer: wgpu::Buffer,
    pub(super) overflow_readback: OverflowReadback,

    /// Region uniforms — `array<RegionUniform>` storage binding,
    /// sized for `MAX_REGIONS` slots.
    pub(super) regions_buffer: wgpu::Buffer,

    /// Per-level uniform buffer with `MAX_DEPTH+1` slots at
    /// `LEVEL_UNIFORM_STRIDE` apart.
    pub(super) level_uniforms_buffer: wgpu::Buffer,

    pub(super) source_hash: u64,
    pub(super) group0_bind_group: Option<wgpu::BindGroup>,
    pub(super) group0_buffers_epoch: u64,
}
