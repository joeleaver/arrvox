...
// ──────────────────────────────────────────────────────────────────
// Coarse-LOD shadow-mesh extraction (Phase 3.1 of the splat-to-mesh
// pivot)
// ──────────────────────────────────────────────────────────────────

/// Walk the octree to a coarser level than finest, collect solid cells
/// at that level, and run surface nets on the level-N grid.
///
/// `lod_levels` is the number of levels above finest. `lod_levels = 4`
/// means the resulting mesh runs on cells `2^4 = 16` finest voxels per
/// axis (4096× volume reduction); the resulting triangle count
/// typically drops by a factor of `~64×–256×` (surface area scales as
/// `N²` while volume scales as `N³`). Sufficient for shadow casting
/// where the shadow-map texel density is the visual bottleneck, not
/// caster geometric detail.
///
/// Vertex normal_oct + leaf_attr_id are deliberately set to 0 — the
/// shadow shader (`mesh_shadow.wesl`) only reads `local_pos`. Skipping
/// the prefilter-attr lookup keeps this fast and self-contained.
///
/// Caller constraints:
///  · `lod_levels >= 2` recommended (puts the LOD level at or above
///    the brick boundary, so BRICKs can never appear *above* the
///    target depth in the walk). `lod_levels = 0` returns an empty
///    mesh; use [`extract_surface_mesh`] for finest extraction.
///  · `lod_levels < octree_depth` — coarser than the root would mean
///    extracting a single cell, which has nothing to surface.
pub fn extract_shadow_mesh_lod(
    octree_nodes: &[u32],
    octree_depth: u8,
    lod_levels: u8,
    base_voxel_size: f32,
    ...
    let mut vertices: Vec<MeshVertex> = Vec::new();
    let mut indices: Vec<u32> = Vec::new();
    if octree_nodes.is_empty() || lod_levels == 0 || lod_levels > octree_depth {
        return (vertices, indices);
    }
    let target_depth = octree_depth - lod_levels;
    let target_voxel_size = base_voxel_size * (1u32 << lod_levels) as f32;

    let mut cells: HashSet<IVec3> = HashSet::new();
    walk_collect_cells_lod(
        octree_nodes,
        brick_cells,
        0,
        UVec3::ZERO,
    ...
    for &cell in &cells {
        for face in 0..6 {
            ...
            // Active edge — emit a quad of 4 SN-cube vertices, CCW
            ...
            for i in 0..4 {
                ...
                quad[i] = match cube_vertex.get(&cube) {
                    Some(&v) => v,
                    None => {
                        let vertex = build_lod_cube_vertex(
                            cube,
                            &cells,
                            target_voxel_size,
                            grid_origin,
...
/// Walk the octree to `target_depth`, populating `cells` with the
/// solid level-N cells. Above `target_depth`, terminator nodes
/// (INTERIOR / LEAF) are expanded to fill the level-N region they
/// cover; BRICK nodes are subdivided per-cell at LOD granularity so
/// the LOD mesh tracks the brick's actual occupancy instead of
/// inflating to a full cube. INTERIOR_NODE-region cells *are*
/// expanded here (unlike the finest extractor, which skips them) —
/// at LOD level the expansion is bounded by `2^target_depth` cells
/// per axis, so memory cost is fine.
fn walk_collect_cells_lod(
    nodes: &[u32],
    brick_cells: &[u32],
    node_idx: usize,
    origin_lod: UVec3,
    ...
    if level >= target_depth {
        // At or past target depth → one solid level-N cell.
        cells.insert(IVec3::new(
            origin_lod.x as i32,
            origin_lod.y as i32,
            origin_lod.z as i32,
        ));
    ...
    if node == INTERIOR_NODE || is_leaf(node) {
        // Pure terminator — fill the entire level-N region it covers.
        // (No way to know finer detail; conservative is correct here.)
        let span = 1u32 << (target_depth - level);
        for dz in 0..span {
            for dy in 0..span {
                for dx in 0..span {
                    cells.insert(IVec3::new(
                        origin_lod.x as i32 + dx as i32,
                        origin_lod.y as i32 + dy as i32,
                        origin_lod.z as i32 + dz as i32,
                    ));
                }
            }
        }
    ...
    if is_brick(node) {
        // BRICK at level < target_depth — subdivide. The brick covers
        // `1 << (target_depth - level)` LOD cells per axis; each LOD
        // cell aggregates `BRICK_DIM / lod_per_brick` finest cells per
        // axis. A LOD cell is solid iff *any* of those finest cells is
        // non-EMPTY. This tracks the brick's actual occupancy rather
        // than inflating it to a full cube — without this step the
        // shadow mesh shows axis-aligned brick-cube tessellation
        // wherever the surface passes through a brick.
        let lod_per_brick = 1u32 << (target_depth - level);
        // BRICK_DIM = 4; the asserts below verify the LOD level fits.
        debug_assert!(
            lod_per_brick <= BRICK_DIM,
            "lod_per_brick {} > BRICK_DIM {} — target_depth too fine for the brick representation",
            lod_per_brick,
            BRICK_DIM,
        );
        let finest_per_lod = BRICK_DIM / lod_per_brick;
        let bid = brick_id(node);
        let brick_base = (bid * BRICK_CELLS) as usize;
        for lz in 0..lod_per_brick {
            for ly in 0..lod_per_brick {
                for lx in 0..lod_per_brick {
                    let mut any_non_empty = false;
                    'check: for cz in 0..finest_per_lod {
                        for cy in 0..finest_per_lod {
                            for cx in 0..finest_per_lod {
                                let bx = lx * finest_per_lod + cx;
                                let by = ly * finest_per_lod + cy;
                                let bz = lz * finest_per_lod + cz;
                                let flat = (bx
                                    + by * BRICK_DIM
                                    + bz * BRICK_DIM * BRICK_DIM)
                                    as usize;
                                if brick_cells[brick_base + flat] != BRICK_EMPTY {
                                    any_non_empty = true;
                                    break 'check;
                                }
                            }
                    ...
                    if any_non_empty {
                        cells.insert(IVec3::new(
                            origin_lod.x as i32 + lx as i32,
                            origin_lod.y as i32 + ly as i32,
                            origin_lod.z as i32 + lz as i32,
                        ));
                    }
                }
            }
    ...
    if is_branch(node) {
        let children_offset = node as usize;
        let half_lod = 1u32 << (target_depth - level - 1);
        for octant in 0u32..8 {
            let dx = octant & 1;
            let dy = (octant >> 1) & 1;
            let dz = (octant >> 2) & 1;
            let child_origin_lod = UVec3::new(
                origin_lod.x + dx * half_lod,
                origin_lod.y + dy * half_lod,
                origin_lod.z + dz * half_lod,
            );
            walk_collect_cells_lod(
                nodes,
                brick_cells,
...
/// Edge-crossing centroid for an SN cube at LOD level. Same algorithm
/// as [`build_cube_vertex`] but skips the leaf_attr_pool / normal
/// averaging — shadow doesn't read those vertex attributes.
fn build_lod_cube_vertex(
    cube: IVec3,
    cells: &HashSet<IVec3>,
    voxel_size: f32,
    grid_origin: Vec3,
    ...
    for i in 0u32..8 {
        let oa = corner_offset(i);
        let c = cube + oa;
        corner_solid[i as usize] = cells.contains(&c);
    }
    let mut crossing_sum = Vec3::ZERO;
    let mut crossing_count: u32 = 0;
    for &(a, b) in &CUBE_EDGES {
...
mod tests {
    ...
    /// LOD extraction at one level above brick (lod_levels = 2) on a
    /// brick whose cells are all populated should treat the whole
    /// brick as a single solid level-N cell — producing a closed
    /// 6-face cube mesh. Verifies the LOD walk correctly aggregates
    /// terminator nodes to the LOD grid.
    #[test]
    fn lod_extraction_collapses_full_brick_to_one_cell() {
        // depth=2 root with a fully-populated BRICK at root.
        let nodes = vec![make_brick(0)];
        let bricks = vec![1u32; BRICK_CELLS as usize];
        // lod_levels = 2 → target_depth = 0. The single root-level BRICK
        // becomes one solid level-N cell at (0, 0, 0). Cell-grid SN
        // emits a closed cube (8 verts, 36 indices) around it.
        let (verts, indices) =
            extract_shadow_mesh_lod(&nodes, 2, 2, 1.0, Vec3::ZERO, &bricks);
        assert_eq!(verts.len(), 8, "8 SN-cube vertices around the level-N cell");
        assert_eq!(indices.len(), 36, "6 faces × 2 triangles × 3 indices");
        // Shadow LOD vertices skip the leaf_attr / normal_oct work.
        for v in &verts {
            assert_eq!(v.leaf_attr_id, 0);
            assert_eq!(v.normal_oct, 0);
        }
    }

    /// `lod_levels = 1` requires per-brick subdivision: the BRICK is
    /// at level `depth - 2`, target_depth = `depth - 1`, so each
    /// brick maps to 2³ LOD cells (each aggregating 2³ finest cells).
    /// With only the (0, 0, 0) finest cell filled inside the brick,
    /// only the (0, 0, 0) LOD cell within the brick should be solid —
    /// not the whole brick inflated to a cube.
    #[test]
    fn lod_extraction_subdivides_bricks_for_lod_1() {
        // depth=3, root = BRANCH; octant 0 = BRICK; rest empty.
        let mut nodes = vec![0u32; 9];
        nodes[0] = 1; // branch with children at offset 1
        nodes[1] = make_brick(0); // -X-Y-Z octant
        for i in 2..9 {
            nodes[i] = EMPTY_NODE;
        }
        let mut bricks = vec![BRICK_EMPTY; BRICK_CELLS as usize];
        bricks[0] = 99; // single solid finest cell at (0, 0, 0) within the brick

        let (verts, indices) =
            extract_shadow_mesh_lod(&nodes, 3, 1, 1.0, Vec3::ZERO, &bricks);

        // One solid LOD cell at (0, 0, 0) → closed cube around it
        // (8 verts, 36 indices). The full brick cube would have given
        // 64 LOD cells solid (bricks[0..63] all marked solid by the
        // old conservative path) and a much different vertex count;
        // this test guards against that regression.
        assert_eq!(
            verts.len(),
            8,
            "subdivision should isolate the single filled finest cell",
        );
        assert_eq!(indices.len(), 36);
    }

    /// LOD with lod_levels = 0 returns nothing — caller should use
    /// `extract_surface_mesh` for finest extraction. lod_levels >
    /// octree_depth is also rejected (would underflow target_depth).
    #[test]
    fn lod_extraction_rejects_invalid_levels() {
        let nodes = vec![make_leaf(1)];
        let (v, i) = extract_shadow_mesh_lod(&nodes, 4, 0, 1.0, Vec3::ZERO, &[]);
        assert!(v.is_empty() && i.is_empty(), "lod_levels=0 returns empty");
        let (v, i) = extract_shadow_mesh_lod(&nodes, 4, 5, 1.0, Vec3::ZERO, &[]);
        assert!(v.is_empty() && i.is_empty(), "lod_levels > depth returns empty");
    }
}
