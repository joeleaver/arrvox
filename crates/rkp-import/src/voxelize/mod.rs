//! Top-level import orchestration: mesh → `.rkp` opacity-octree
//! asset (with optional `.rkskel` sidecar).
//!
...
pub fn import_mesh_to_opacity_rkp_with(
    ...
    eprintln!(
        "[rkp-import] emit_shell_leaves: {} shell voxels (from {} interior bricks, depth {})",
    ...
    // ran at asset-load time in `rkp-render::rkp_scene_manager`;
    ...
    if shell_output.voxel_count > 0 {
        ...
        let smoothed = rkp_core::laplacian_smooth::smooth_shell_normals_raw(
            &octree,
    ...
    // The `.rkp` header carries a 16-entry material palette. Voxel
    ...
    if mesh.materials.len() > 16 {
        reporter.report(ImportEvent::Warn {
            message: format!(
                "mesh has {} materials; only the first 16 fit in the .rkp palette header (voxel material IDs are unaffected — they index the project MaterialPalette at runtime)",
                mesh.materials.len()
            ),
        });
    }
