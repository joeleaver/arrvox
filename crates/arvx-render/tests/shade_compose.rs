...
fn empty_user_chunk_validates() {
    let src = arvx_render::arvx_shade::compose_shade_source("");
...
"#,
    ...
    assert!(chunks.shade.contains("arvx_user_1_shade"));
    assert!(chunks.shade.contains("dispatch_user_shade"));

    let src = arvx_render::arvx_shade::compose_shade_source(&chunks.shade);
    ...
    assert!(src.contains("arvx_user_1_shade"));
