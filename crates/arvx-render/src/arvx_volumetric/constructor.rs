//! `ArvxVolumetricPass::new` — wgpu pipeline + buffer + texture setup
...
use super::{CloudParams, ArvxVolumetricPass, VolumetricParams};
...
impl ArvxVolumetricPass {
    pub fn new(device: &wgpu::Device, width: u32, height: u32) -> Self {
        ...
        // Fog march pipeline.
        let fog_module = compile_pass_shader(device, wesl::include_wesl!("arvx_fog_march"), "arvx_fog_march");
        ...
        let cloud_module = compile_pass_shader(device, wesl::include_wesl!("arvx_cloud_march"), "arvx_cloud_march");
        ...
        // Composite pipeline.
        let composite_module = compile_pass_shader(device, wesl::include_wesl!("arvx_vol_composite"), "arvx_vol_composite");
        ...
        let history_update_module = compile_pass_shader(
            device, wesl::include_wesl!("arvx_vol_history_update"), "arvx_vol_history_update",
        );
        ...
        let sun_atten_module = compile_pass_shader(
            device, wesl::include_wesl!("arvx_cloud_sun_atten"), "arvx_cloud_sun_atten",
        );
