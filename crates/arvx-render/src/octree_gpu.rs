...
//!
//! Arrvox reinterprets the following GpuObject fields for octree objects:
//!
...
use arvx_core::{OctreeAllocator, OctreeHandle, SparseOctree};
...
pub struct OctreeGpu {
    ...
    /// Live handles for all allocated octrees (for building ArvxGpuObjects).
...
impl OctreeGpu {
    ...
    pub fn allocate_raw(&mut self, nodes: &[u32], depth: u8, base_voxel_size: f32) -> OctreeHandle {
        let octree = arvx_core::SparseOctree::from_raw(nodes, depth, base_voxel_size);
        let handle = self.allocator.allocate(&octree);
        self.live_handles.push(handle.clone());
        handle
    }
    ...
    /// by `ArvxSceneManager::walk_snapshot` so the painted-material walk
    ...
    /// The GPU binding (see [`ArvxScene`]) interleaves these with the node
    ...
    /// Apply a [`arvx_core::sparse_octree::OctreeMutationLog`] (typically
    /// produced by [`arvx_core::sculpt::apply_delta`]) to the packed
    ...
    pub fn apply_mutation_log(
        &mut self,
        handle: &OctreeHandle,
        log: &arvx_core::sparse_octree::OctreeMutationLog,
    ) {
    ...
    pub fn dirty_ranges(&self) -> &arvx_core::DirtyRanges {
    ...
    pub fn dirty_ranges_mut(&mut self) -> &mut arvx_core::DirtyRanges {
