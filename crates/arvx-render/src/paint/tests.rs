use super::*;
use glam::{UVec3, Vec3};
use arvx_core::brick_pool::{BrickPool, BRICK_DIM};
use arvx_core::leaf_attr_pool::LeafAttrPool;
use arvx_core::sparse_octree::SparseOctree;
use arvx_core::OctreeAllocator;
...
fn paint_material_full_weight_overwrites() {
    ...
    *pool.get_mut(0) = arvx_core::LeafAttr::new(Vec3::Y, 1);
...
fn paint_material_partial_weight_blends_into_secondary() {
    ...
    *pool.get_mut(0) = arvx_core::LeafAttr::new(Vec3::Y, 3);
...
fn paint_material_noop_when_already_matching_primary() {
    let mut pool = LeafAttrPool::new(8);
    pool.allocate_range(1).unwrap();
    *pool.get_mut(0) = arvx_core::LeafAttr::new(Vec3::Y, 5);
...
fn paint_material_zero_weight_is_noop() {
    let mut pool = LeafAttrPool::new(8);
    pool.allocate_range(1).unwrap();
    *pool.get_mut(0) = arvx_core::LeafAttr::new(Vec3::Y, 9);
...
fn slab_octree(
    ...
    let brick_node = arvx_core::sparse_octree::make_brick(brick_id);
