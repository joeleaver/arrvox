...
//!
//! [`ArvxRenderer`] holds the scene-wide buffers (scene, atmosphere LUTs,
...
//!
//! The caller (arvx-editor) drives each frame:
//! 1. Build ArvxGpuObjects from ECS/scene state.
...
use crate::arvx_scene::{ArvxScene, GeometryUpload, FrameUpload};
use crate::arvx_shade::{ShadeParams, GpuLight, GpuMaterial};
use crate::arvx_atmosphere::ArvxAtmospherePass;
...
/// The Arrvox renderer — shared state only. Per-viewport passes live
/// in [`ViewportRenderer`].
pub struct ArvxRenderer {
    ...
    pub atmosphere: ArvxAtmospherePass,
    ...
    /// march does, so the existing `arvx_glass` composite runs
    /// unchanged.
    ...
    /// `ARVX_MESH_GLASS_DEBUG_FORCE` snapshot (read once at startup).
    ...
    /// `ARVX_RASTER_DIAG=1` per-asset cluster aggregate counts captured
    ...
    /// `ARVX_RASTER_DIAG=1` print to surface the per-LOD admit shape
...
/// `ARVX_RASTER_DIAG=1` aggregated cluster-state snapshot captured at
...
struct MeshClusterDiag {
    ...
    /// `ARVX_MESH_LOD_STATS` to see how much LOD culling is happening.
    total_indices: u32,
}
...
impl ArvxRenderer {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, _width: u32, _height: u32) -> Self {
        let scene = ArvxScene::new(device);
        ...
        let shade_params_buffer = Self::create_init_buffer(device, "arvx_shade_params",
            wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            bytemuck::bytes_of(&default_params));
        let lights_buffer = Self::create_init_buffer(device, "arvx_shade_lights",
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
            bytemuck::bytes_of(&default_light));
        let materials_buffer = Self::create_init_buffer(device, "arvx_shade_materials",
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
            bytemuck::bytes_of(&default_material));

        let atmosphere = ArvxAtmospherePass::new(device);
        ...
        let mesh_glass_debug_force = std::env::var("ARVX_MESH_GLASS_DEBUG_FORCE")
            .ok()
        ...
        if mesh_glass_debug_force != 0 {
            ...
            eprintln!("[ArvxRenderer] ARVX_MESH_GLASS_DEBUG_FORCE={mesh_glass_debug_force} — {mode}");
        }
    ...
    /// `ARVX_CSM_THRESHOLD_FALLOFF` env-var path (still honored as a
    ...
    /// `ArvxSceneManager::asset_mesh`, and `dispatch_index_count` —
    ...
    /// `ArvxSceneManager::iter_loaded_asset_clusters`.
    ///
    ...
    pub fn upload_mesh_clusters_for_asset(
        ...
        // ARVX_RASTER_DIAG=1 — aggregate per-LOD counts at upload time
        ...
        // default config.
        if std::env::var("ARVX_RASTER_DIAG").is_ok() {
    ...
    pub fn upload_proxy_mesh_for_asset(
        &mut self,
        handle_raw: u32,
        vertices: &[arvx_core::mesh_extract::ProxyVertex],
        indices: &[u32],
    ) {
    ...
    pub fn dispatch_mesh(
        ...
        // sites.
        let primary_disabled = std::env::var("ARVX_MESH_DISABLE_PRIMARY").is_ok();
        ...
        // `ARVX_MESH_GLASS_DEBUG_FORCE=2` raises the FS opacity gate
        ...
        //
        // `ARVX_MESH_LOD_THRESHOLD` overrides the compile-time default
        ...
        let pixel_threshold_primary = pixel_threshold_env(
            "ARVX_MESH_LOD_THRESHOLD",
            PIXEL_THRESHOLD_PRIMARY,
        );
        let lod_stats_enabled = std::env::var("ARVX_MESH_LOD_STATS").is_ok();
        let pipestats_enabled = std::env::var("ARVX_MESH_PIPESTATS").is_ok();
        let force_admit_flag: u32 = if std::env::var("ARVX_MESH_DEBUG_FORCE_ADMIT").is_ok() {
            1
        } else {
            0
        };
        // ARVX_MESH_DEBUG_FORCE_LEVEL=N: bypass Karis admit and admit
        ...
        let force_level: u32 = std::env::var("ARVX_MESH_DEBUG_FORCE_LEVEL")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(u32::MAX);
        ...
        // `ARVX_MESH_DEBUG_DIRECT=1` bypasses Phase 6.2/6.3 entirely:
        ...
        let direct_mode = std::env::var("ARVX_MESH_DEBUG_DIRECT").is_ok();
        ...
        if !direct_mode && !primary_disabled {
            // Stats lifecycle (ARVX_MESH_LOD_STATS=1 only):
        ...
        // ARVX_MESH_STATS=1 prints per-frame mesh stats with a per-asset
        // breakdown, mirroring ARVX_SPLAT_STATS. The cluster counts are
        ...
        if std::env::var("ARVX_MESH_STATS").is_ok() {
        ...
        // ARVX_RASTER_DIAG=1 — per-frame, per-active-asset cluster state.
            ...
            if f % 60 == 0 {
                ...
                for d in draws {
                    ...
                    match diag {
                        ...
                        None => {
                            eprintln!(
                                "[raster_diag frame] asset={} total_clusters={} (diag not captured — upload before setting ARVX_RASTER_DIAG?)",
            ...
            for (slot, d) in draws.iter().enumerate() {
                ...
                if direct_mode {
                    ...
                    let cluster_count = self
                        ...
                        .map(|(_, c)| c)
                        .unwrap_or(0);
                    // `ARVX_MESH_DEBUG_MAX_DRAWS` caps the number of
                    ...
                    let max_draws = std::env::var("ARVX_MESH_DEBUG_MAX_DRAWS")
                        .ok()
                        .and_then(|s| s.parse::<u32>().ok())
    ...
    pub fn dispatch_mesh_shadow(
        ...
        if std::env::var("ARVX_MESH_DISABLE_SHADOW").is_ok() {
            return;
        }
        ...
        // `ARVX_MESH_DEBUG_DIRECT=1` also bypasses the shadow LOD
        ...
        // visible. Override with `ARVX_MESH_SHADOW_LOD_THRESHOLD`
        // (base) and `ARVX_CSM_THRESHOLD_FALLOFF` (per-cascade scale).
        const PIXEL_THRESHOLD_SHADOW: f32 = 6.0;
        let base_threshold = pixel_threshold_env(
            "ARVX_MESH_SHADOW_LOD_THRESHOLD",
            PIXEL_THRESHOLD_SHADOW,
        );
        // Per-cascade falloff: env var (CI / headless) takes precedence
        // over the engine-set value so ARVX_CSM_THRESHOLD_FALLOFF still
        ...
        let threshold_falloff = std::env::var("ARVX_CSM_THRESHOLD_FALLOFF")
            .ok()
            .and_then(|s| s.parse::<f32>().ok())
        ...
        let lod_stats_enabled = std::env::var("ARVX_MESH_LOD_STATS").is_ok();
        let pipestats_enabled = std::env::var("ARVX_MESH_PIPESTATS").is_ok();
        let force_admit = std::env::var("ARVX_MESH_DEBUG_FORCE_ADMIT").is_ok();
        let force_level: u32 = std::env::var("ARVX_MESH_DEBUG_FORCE_LEVEL")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(u32::MAX);
        let max_draws_override = std::env::var("ARVX_MESH_DEBUG_MAX_DRAWS")
            .ok()
            .and_then(|s| s.parse::<u32>().ok());
    ...
    pub fn render_to(
        ...
        if !raymarch && std::env::var("ARVX_MESH_PIPESTATS").is_ok() {
