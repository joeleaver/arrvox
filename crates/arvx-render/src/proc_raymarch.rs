...
//! `arvx_procedural::flatten`) — a post-order RPN encoding of the tree.
...
//!
//! This pass owns no shared state with `ArvxScene`: it binds only the
...
impl ProcRaymarchPass {
    pub fn new(device: &wgpu::Device) -> Self {
        // Group 0: camera uniform. Mirrors `ArvxScene`'s camera binding
        ...
        // normal, material) plus a 4th arvx-side pick texture. The pick
        // slot holds the hit primitive's NodeId so packed_r's high 16
        // bits on the shared material G-buffer stay available for
        // `secondary_material_id` — which is what `arvx_shade` reads
        // for dual-material lerp. Not part of the shared GBuffer
        // because it's arvx-specific; see `ViewportRenderer::pick_view`.
    ...
    /// a G-buffer resize. `pick_view` is the arvx-side `R32Uint` pick
    ...
    pub fn upload_instructions(
        ...
        self.has_warps = instructions.iter().any(|ins| {
            ins.op == arvx_procedural::OpKind::PushNoiseDisplace as u32
                || ins.op == arvx_procedural::OpKind::PushMirror as u32
                || ins.op == arvx_procedural::OpKind::PushArray as u32
        });
