//! `ArvxSceneManager` — the central CPU-side scene type. Holds the
...
pub struct ArvxSceneManager {
    ...
    /// sentinel (see `arvx_core::brick_face_links`). Sized to cover
    ...
    /// Cache of loaded .arvx assets keyed by canonical file path. Instances
    ...
    /// and uploaded by the render thread inside `ArvxScene::upload_frame`,
...
impl ArvxSceneManager {
    ...
    /// `ArvxSceneManager` instance would create a fresh epoch atomic
    ...
    pub(super) fn merge_face_links(&mut self, rows: &[[u32; 6]]) {
        ...
        if self.brick_face_links.len() < rows.len() {
            self.brick_face_links.resize(
                rows.len(),
                [arvx_core::brick_face_links::FACE_EMPTY; 6],
            );
        }
        ...
        for (i, row) in rows.iter().enumerate() {
            if row.iter().any(|&v| v != arvx_core::brick_face_links::FACE_EMPTY) {
    ...
    /// through the `Arc<Mutex<ArvxSceneManager>>`, which means *they
    ...
    pub fn geometry_upload(&self) -> GeometryUpload<'_> {
        ...
        GeometryUpload {
            ...
            brick_face_links: arvx_core::brick_face_links::as_bytes(&self.brick_face_links),
            octree_dirty,
            leaf_attr_dirty,
            color_dirty,
            bone_dirty,
    ...
    /// worker after `ArvxScene::upload_geometry` succeeds — the upload
    ...
    pub fn deallocate_spatial(&mut self, handle: &arvx_core::scene_node::SpatialHandle) {
        self.bump_geometry_epoch();
