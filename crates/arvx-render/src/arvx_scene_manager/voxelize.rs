...
//!
//! Sibling impl block on `ArvxSceneManager`. Procedural primitives and
...
impl ArvxSceneManager {
    pub fn voxelize_primitive(
        &mut self,
        primitive: &arvx_core::scene_node::SdfPrimitive,
        material_id: u16,
        voxel_size: f32,
        ...
        use arvx_core::scene_node::SdfPrimitive;
        ...
        let sdf_fn: Box<dyn Fn(glam::Vec3) -> f32> = match primitive {
            SdfPrimitive::Box { half_extents: he } => {
                ...
                Box::new(move |pos| arvx_core::evaluate_primitive(&scaled, pos))
            }
            _ => {
                ...
                Box::new(move |pos| arvx_core::evaluate_primitive(&prim, pos * inv_scale) * min_scale)
            }
        };
        ...
        let spatial = arvx_core::scene_node::SpatialHandle::Octree {
    ...
    /// Integrate a self-contained [`arvx_core::BakeArtifact`] (produced
    ...
    pub fn integrate_artifact(
        &mut self,
        mut artifact: arvx_core::BakeArtifact,
        aabb: &arvx_core::Aabb,
        voxel_size: f32,
    ) -> Option<VoxelizeResult> {
        self.bump_geometry_epoch();
        use arvx_core::brick_face_links::{FACE_EMPTY, FACE_INTERIOR};
        use arvx_core::brick_pool::{BRICK_EMPTY, BRICK_INTERIOR};
    ...
    pub fn deallocate_geometry(
        &mut self,
        spatial: &arvx_core::OctreeHandle,
