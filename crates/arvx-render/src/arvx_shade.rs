//! Arrvox deferred PBR shading compute pass.
//!
...
pub struct ArvxShadePass {
    ...
    /// consumed by the screen-space paint cursor in `arvx_shade.wesl`.
...
/// frame and consumed by `arvx_shade.wesl` to draw the screen-space
...
impl ArvxShadePass {
    pub fn new(device: &wgpu::Device, width: u32, height: u32) -> Self {
        ...
        let ssao_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("arvx_shade shadow+ssao+shadowmap"),
                entries: &[
                    texture_entry(0),
                    ...
                    // `glass_shadow_attenuation` in `arvx_shade.wesl`.
                    wgpu::BindGroupLayoutEntry {
                        binding: 3,
                        visibility: wgpu::ShaderStages::COMPUTE,
                        ty: wgpu::BindingType::Texture {
        ...
        let brush_state_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("arvx_shade brush_state"),
                entries: &[wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
        ...
        let shader_params_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("arvx_shade shader_params"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("arvx_shade pipeline"),
            bind_group_layouts: &[
    ...
    pub fn upload_shader_params(
        ...
        if !slots.is_empty() {
            ...
            let hash = super::arvx_renderer::d4_hash_bytes(bytes);
    ...
    pub fn set_brush_state(&mut self, device: &wgpu::Device, brush_state_buffer: &wgpu::Buffer) {
        self.brush_state_bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("arvx_shade brush_state bg"),
    ...
    pub fn set_shadow_and_ssao(
        ...
        self.ssao_bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("arvx_shade shadow+ssao+shadowmap bg"),
    ...
    pub fn set_shade_data(
        ...
        self.shade_bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("arvx_shade params bg"),
    ...
    pub fn set_camera(&mut self, device: &wgpu::Device, camera_buffer: &wgpu::Buffer) {
        self.camera_bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("arvx_shade camera bg"),
    ...
    fn create_output_bind_group(
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("arvx_shade output bg"),
            layout,
            entries: &[wgpu::BindGroupEntry {
                binding: 0,
...
/// `_END` const-decl anchors in `arvx_shade.wgsl`. Pass `""` for the
/// no-shaders case (default identity stub stays put).
pub fn compose_shade_source(user_chunk: &str) -> String {
    let shade_src = wesl::include_wesl!("arvx_shade");
...
fn build_shade_pipeline(
    ...
    let module = crate::compile_pass_shader(device, &source, "arvx_shade");
...
mod tests {
    ...
    #[test]
    fn arvx_shade_shader_is_valid_wgsl() {
