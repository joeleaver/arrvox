//! Glass composite post-pass — runs after `arvx_shade` and before the
...
impl ArvxGlassPass {
    pub fn new(device: &wgpu::Device, width: u32, height: u32) -> Self {
        ...
        let module = compile_pass_shader(device, wesl::include_wesl!("arvx_glass"), "arvx_glass");

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("arvx_glass"),
            layout: Some(&device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("arvx_glass pipeline layout"),
    ...
    fn create_output(device: &wgpu::Device, w: u32, h: u32) -> (wgpu::Texture, wgpu::TextureView) {
        let tex = device.create_texture(&wgpu::TextureDescriptor {
            ...
            usage: wgpu::TextureUsages::STORAGE_BINDING
                 ...
                 // bloom_composite input — see arvx_renderer.rs's
...
mod tests {
    #[test]
    fn arvx_glass_shader_is_valid_wgsl() {
        let src = wesl::include_wesl!("arvx_glass");
