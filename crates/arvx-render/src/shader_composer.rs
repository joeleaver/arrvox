...
//! into `arvx_shade.wgsl` (Phase B) and the geometry-build pipeline
...
//! `arvx_render::shader_composer::Foo` keeps working unchanged.
//!
//! ## Authoring contract
//!
...
//!    `arvx_user_<id>_<hook>` so dispatch can call it by a stable name
//!    independent of the user's choice of `<name>`.
//! 3. Four `dispatch_user_*` switches are emitted, one per hook. Each
//!    switch routes by `shader_id` to the matching `arvx_user_<id>_<hook>`
...
// Public re-exports — keep `arvx_render::shader_composer::Foo` stable.
pub use compose::{compose, compose_mesh_path_pipeline_sources, splice_const_marker};
pub use hash::fnv1a_64;
pub use parser::{parse_file, scan_dir};
pub use types::{
    ComposedChunks, GeometryDecl, ParamDef, ShaderComposerError, ShaderMetadata,
    SpawnCountCache, UserShaderEntry, UserShaderInfo, UserShaderRegistry,
};
