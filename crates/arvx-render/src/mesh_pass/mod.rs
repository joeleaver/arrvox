...
// in arvx-core (`mesh_extract`, `mesh_cluster`, `mesh_lod`) so
// `arvx-import` (write-time bake) and the procedural bake path
// (`arvx_core::asset_file::write_artifact_rkp`) can both serialise
// the result into `.arvx` and skip the rebuild at editor load. The
// re-exports here keep call sites in `arvx-render` from churning.
pub use arvx_core::mesh_cluster::{
    cluster_mesh, MeshletCluster, MAX_TRIS_PER_CLUSTER, MAX_VERTS_PER_CLUSTER,
    PARENT_GROUP_ERROR_ROOT,
};
pub use arvx_core::mesh_extract::{extract_surface_mesh, MeshVertex};
pub use arvx_core::mesh_lod::{build_cluster_dag, ClusterDag, LOD_LEVELS};
